...
public sealed class GameService(QsmpdleDbContext Qsmp, IMemoryCache Cache) : IGameService
{
    ...
    public async Task<Character> GetDailyMemberAsync(CancellationToken cancellationToken)
    {
        var today = DateOnly.FromDateTime(DateTime.UtcNow);
    ...
    private async Task<Character> GetRandomMemberAsync() => Qsmp.Members.ToList().ElementAt(Random.Shared.Next(Qsmp.Members.Count()));
